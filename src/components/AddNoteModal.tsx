...
import { FileText, Camera, Mic, Upload, Play, Pause, Square, X } from 'lucide-react';
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[90vw] max-w-md mx-auto h-[90vh] max-h-[600px] flex flex-col p-4">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="text-lg">Add Note to Page {pageNumber}</DialogTitle>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
          <TabsList className="grid w-full grid-cols-3 flex-shrink-0">
            <TabsTrigger value="text" className="flex items-center space-x-1 text-xs">
          ...
          <div className="flex-1 overflow-auto">
            <TabsContent value="text" className="space-y-3 mt-3">
              <div className="space-y-2">
                <Label htmlFor="textNote" className="text-sm">Text Note</Label>
                <Textarea
                  id="textNote"
            ...
            <TabsContent value="photo" className="space-y-3 mt-3">
              <div className="space-y-3">
                ...
                {/* Photo Caption */}
                <div className="space-y-2">
                  <Label htmlFor="photoCaption" className="text-sm">Caption</Label>
                  <Input
                    id="photoCaption"
              ...
              <div className="flex space-x-2 pt-2">
                <Button 
                  ...
                  className="flex-1 gradient-bg text-sm"
                  disabled={(!selectedFile && !photoCaption.trim()) || isUploading}
                >
            ...
            <TabsContent value="voice" className="space-y-3 mt-3">
              <div className="space-y-3">
                ...
                <div className="border-2 border-gray-200 rounded-lg p-4 text-center">
                  {!isRecording && !recordedBlob && (
                    <div className="space-y-3">
                      ...
                      <p className="text-sm text-gray-600">Record a voice note</p>
                      <Button onClick={startRecording} className="gradient-bg text-sm">
                  ...
                  {isRecording && (
                    <div className="space-y-3">
                      ...
                      <p className="text-red-600 font-medium text-sm">Recording... {formatTime(recordingTime)}</p>
                      <div className="flex space-x-2 justify-center">
                        <Button onClick={stopRecording} variant="outline" size="sm" className="text-xs">
                          <Square className="w-3 h-3 mr-1" />
                          Stop
                        </Button>
                        <Button onClick={cancelRecording} variant="outline" size="sm" className="text-xs">
                          Cancel
                        </Button>
                      </div>
                    </div>
                  ...
                  {!isRecording && recordedBlob && (
                    <div className="space-y-3">
                      ...
                      <div className="flex space-x-2 justify-center">
                        <Button onClick={playPauseAudio} variant="outline" size="sm" className="text-xs">
                          {isPlaying ? <Pause className="w-3 h-3 mr-1" /> : <Play className="w-3 h-3 mr-1" />}
                          {isPlaying ? 'Pause' : 'Play'}
                        </Button>
                        <Button onClick={startRecording} variant="outline" size="sm" className="text-xs">
                          Re-record
                        </Button>
                      </div>
                    </div>
                ...
                {/* Voice Caption */}
                <div className="space-y-2">
                  <Label htmlFor="voiceCaption" className="text-sm">Description</Label>
                  <Input
                    id="voiceCaption"
              ...
              <div className="flex space-x-2 pt-2">
                <Button 
                  ...
                  className="flex-1 gradient-bg text-sm"
                  disabled={(recordingTime === 0 && !voiceCaption.trim()) || isUploading}
                >
